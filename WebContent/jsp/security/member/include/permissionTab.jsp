...
function chooseProgramDialog(elName, obj) {
	...
	jQuery('input[name=' + elName + ']:checked').each(function(i) {
		...
		if (obj[this.id].operatorType == "F") {
			...
			elmArr[0] += "<input type='hidden' name='member.listProgram["+index+"].id' value='"+obj[this.id].currentId+"' />" ;
			elmArr[0] += "<input type='hidden' name='member.listProgram["+index+"].deleteFlag' />" ;
			elmArr[0] += "<input type='hidden' name='member.listProgram["+index+"].currentId' value='"+obj[this.id].currentId+"'/>";
			elmArr[0] += "<input type='hidden' name='member.listProgram["+index+"].parentId' value='"+obj[this.id].parentId+"'/>";
			elmArr[0] += "<input type='hidden' name='member.listProgram["+index+"].parentIds' value='"+obj[this.id].parentIds+"'/>";
			elmArr[0] += "<input type='hidden' name='member.listProgram["+index+"].operatorId' value='"+obj[this.id].currentId+"'/>";
			elmArr[0] += "<input type='hidden' name='member.listProgram["+index+"].systemName' value='"+obj[this.value.split('_')[0]].label+"'/>";
			elmArr[0] += "<input type='hidden' name='member.listProgram["+index+"].menuName' value='"+getParentNamefn(this.value, obj)+ "'/>";
			elmArr[0] += "<input type='hidden' name='member.listProgram["+index+"].functionName' value='"+obj[this.id].label+"'/>";
...
<s:set var="listTableData" value='%{"member.listProgram"}'/> <!-- domain list เช่น userData.listPermission -->
